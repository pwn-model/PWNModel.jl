...
function format_ns(ns::Float64)::String
    units = "ns"
    value = ns
    if value >= 2000
        value /= 1000
...
function table_to_html(data::Vector{CompareRow})::String
    ...
    for r in data
        ...
        html *= @sprintf("""
            ...
            """, r.n, format_ns(r.time_ns_a), format_ns(r.time_ns_b), emoji, r.factor, r.allocs, r.bytes)
...
function compare_multi_tables(a::Vector{Vector{Row}}, b::Vector{Vector{Row}})::Vector{CompareRow}
    ...
    for r in eachindex(compare_multi[1])
        ...
        out.time_ns_b /= count
        out.factor /= count
        out.allocs = round(Int, out.allocs / count)
        out.bytes = round(Int, out.bytes / count)
        push!(data, out)
    end
